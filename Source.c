...
{
	/*L30-41 : Initialisation des variables (int = entier; double = flottant; char = caract�re(s)*/
	int val_A = 0;
	int val_B = 0;
	int val_C = 0;
	int delta = 0;
	...
	/*L44-54 : Demande � l'utilisateur de rentrer les valeurs de A, B et C*/
	printf("=================\n");
	...
	scanf_s("%d", &val_A);
	printf("Valeur de B : ");
	scanf_s("%d", &val_B); //L'instruction scanf_s permet de r�cup�rer les informations entr�es par l'utilisateur
	printf("Valeur de C : ");
	scanf_s("%d", &val_C);
	getchar(); /*Force le programme � rester ouvert une fois que l'execution atteint la partie conditionnelle. Sans,
			   le programme se fermerait brusquement apr�s que l'utilisateur ait entr� les 3 valeurs*/
	//////////////////////////////////////////////////////////////////////////
	if (val_A <= 0)
	{
		system("cls");
	...
	if (delta > 0)
	{
		...
		alpha = -val_B / 2 * val_A; //Calcul de Alpha, premi�re valeur de la forme Conique
		beta = -delta / 4 * val_A; //Calcul de Beta, derni�re valeur de la forme Conique
		green();
	...
	else if (delta == 0)
	{
		...
		solx0 = (2 * val_A);
		solx0 = -val_B / solx0;
		alpha = solx0; //Calcul de Alpha, premi�re valeur de la forme Conique (� savoir que Alpha est �gal � la Solution 0 lorsque Delta est �gal � 0
		beta = -delta / 4 * val_A; //Calcul de Beta, derni�re valeur de la forme Conique
		green();
